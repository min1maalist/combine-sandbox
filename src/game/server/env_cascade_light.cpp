...
void CLightOrigin::Spawn()
{
	if (angFEnv)
	{
		...
		if (pEntity)
		{
			...
			DevMsg("[Combine Sandbox CSM]		light_environment Founded!\n");
		}
		else
		{
			DevMsg("[Combine Sandbox CSM]		Cant find light_environment with targetname!\n");
		}
	}
}
...
CEnvCascadeLightThird::CEnvCascadeLightThird(void)
{
	...
	Q_strcpy(m_SpotlightTextureName.GetForModify(), "tools\\mask_ring");
...
CEnvCascadeLight::CEnvCascadeLight(void)
{
	...
	Q_strcpy(m_SpotlightTextureName.GetForModify(), "tools\\mask_center");
	m_nSpotlightTextureFrame = 0;
	m_flAmbient = 0.0f;
	m_flNearZ = csm_nearz.GetFloat();
	m_flFarZ = csm_farz.GetFloat();
...
void CEnvCascadeLight::Preparation()
{
	...
	{
		DevMsg("[Combine Sandbox CSM]		Main csm entity can't find \"csmorigin\" entity!");
	}
