...
void CSquirrelVM::ConvertToVariant( HSQUIRRELVM pVM, HSQOBJECT const &pValue, ScriptVariant_t *pVariant )
{
	switch ( sq_type( pValue ) )
	{
		...
		case OT_INSTANCE:
		{
			...
			{
				*pVariant = new QAngle();
				V_memcpy( (void *)pVariant->m_pAngle, pInstance, sizeof( QAngle ) );
				pVariant->m_flags |= SV_FREE;
...
void CSquirrelVM::PushVariant( HSQUIRRELVM pVM, ScriptVariant_t const &Variant )
{
	switch ( Variant.m_type )
	{
		...
		case FIELD_EHANDLE:
		{
			sq_pushehandle( pVM, (CBaseHandle *)Variant.m_pEHandle );
			break;
		}
		case FIELD_HSCRIPT:
		{
		...
		case FIELD_QANGLE:
		{
			...
			V_memcpy( pAngle, Variant.m_pAngle, sizeof(QAngle) );
...
void CSquirrelVM::VariantToString( ScriptVariant_t const &Variant, char( &szValue )[512] )
{
	switch ( Variant.m_type )
	{
		...
		case FIELD_VECTOR:
			...
			break;
		case FIELD_QANGLE:
			V_snprintf( szValue, sizeof( szValue ), "QAngle( %f, %f, %f )", Variant.m_pAngle->x, Variant.m_pAngle->y, Variant.m_pAngle->z );
			break;
		case FIELD_QUATERNION:
